...
class RAMEditCtrl {
    ...
    public function validateSave() {
        ...
        $this->form->ddr = $v->validateFromPost('ddr',[
            'trim' => true,
            'required' => true,
            'required_message' => 'Podaj ddr',
            'min_length' => 4,
            'max_length' => 4,
            'validator_message' => 'Poprawna wartość to (np) "DDRX"'
        ]);
    ...
    public function action_NewRAM() {
        if(RoleUtils::inRole('admin')){
            $this->generateView();
        }
    }
    public function action_RAMEdit() {
        if(RoleUtils::inRole('admin')){
            if ($this->validateEdit()) {
                try {
                    ...
                    $ddr = App::getDB()->get("wartosc_parametrow","*", [
                        "id_towar" => $this->form->id,
                        "id_nazwa_parametrow" => 8
                    ]);
                    ...
                    $cena = App::getDB()->get("wartosc_parametrow","*", [
                        ...
                        $this->form->ddr = $ddr['wartosc_parametrow'];
    ...
    public function action_RAMSave() {
        if(RoleUtils::inRole('admin')){
    ...
    public function generateView(){
        App::getSmarty()->assign('page_title', 'Dodaj pamięć RAM');
        App::getSmarty()->assign('page_description','Naciśnij przycisk, aby przejść do dodania pamięci RAM!');
        
        App::getSmarty()->assign('form', $this->form);
        App::getSmarty()->display('AddRAM.tpl');
    }    
}
